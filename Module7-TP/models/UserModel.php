...
class User {
    ...
    // Méthodes pour gérer les utilisateurs (inscription, connexion, mise à jour, etc.)
    public function registerUser($nom, $prenom, $adresse, $email, $password, $confirmPassword) {
        $pdo = $this->conn; // Obtenir la connexion à la base de données
    ...
    try {
        ...
        if ($existingUser) {
            ...
            $stmt = $pdo->prepare("INSERT INTO utilisateurs (id,nom, prenom, adresse, email, password, role) VALUES (?,?, ?, ?, ?, ?, ?)");
            $stmt->execute([4,$nom, $prenom, $adresse, $email, $hashedPassword, $defaultRole]);
            return true;
        }
    } catch (Exception $e) {
